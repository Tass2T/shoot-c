...
{
    if (SDL_QueryTexture(texture, NULL, NULL, &backgroundRect.w, &backgroundRect.h) != 0)
    {
    ...
    if (SDL_RenderCopy(pRenderer, texture, NULL, &backgroundRect) != 0)
    {
...
void drawRectangle () {
    ...
    SDL_RenderCopy(pRenderer, background, NULL, &backgroundRect);
    SDL_RenderPresent(pRenderer);
